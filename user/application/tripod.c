...
/*******************************************************************************
    ...
    *******************************************************************************/
pid_controller_t downyaw_pid{1000,0,0}; //大yaw的pid，比例系数先初始化为1000，之后调试时再修改
pid_controller_t upyaw_pid{1000,0,0}; //小yaw的pid，比例系数先初始化为1000，之后调试时再修改
pid_controller_t pitch_pid{1000,0,0}; //pitch的pid，比例系数先初始化为1000，之后调试时再修改
rc_data_t RC_CtrlData;
...
upyaw_pid.setpoint = 0;
downyaw_pid.setpoint = 0;
...
void StartTripod(void *argument)
{
  ...
  {
    GetTripodData();
    upyaw_data = (uint16_t)pid_compute(&upyaw_pid, upyaw_angle);
